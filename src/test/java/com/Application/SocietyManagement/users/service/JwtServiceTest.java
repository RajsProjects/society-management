...
class JwtServiceTest {
    ...
    class GenerateToken {
        ...
        void generateToken_containsAllRequiredClaims() {
            String token = jwtService.generateToken(residentUser);
            assertThat(jwtService.extractEmail(token)).isEqualTo("resident@test.com");
            assertThat(jwtService.extractRole(token)).isEqualTo("RESIDENT");
            assertThat(jwtService.extractSocietyId(token)).isEqualTo("society-001");
        }
    }
